...
//                Log.d(TAG, "Updated user: " + Criuze.g() + " from the schema.");
    ...
    public ArrayList<Cabins_TMP> cabinTempList(long uniqCruizeID) {
        ...
        try {
            ...
            openReadableDb();
            Cabins_TMPDao cabins_tmpDao = daoSession.getCabins_TMPDao();
            QueryBuilder<Cabins_TMP> queryBuilder = cabins_tmpDao.queryBuilder().where(Cabins_TMPDao.Properties.CruizeId.eq(uniqCruizeID)).orderAsc(Cabins_TMPDao.Properties.CabinNumber);
...
//                Log.d(TAG, "Updated user: " + Criuze.getName() + " from the schema.");
    ...
    public ArrayList<Cabins_TMP> getGuestDeatilByExcursionCruizeID(long excursionUniqId, long cruizeID) {
        ...
        try {
            openReadableDb();
            Cabins_TMPDao cabins_tmpDao = daoSession.getCabins_TMPDao();
            QueryBuilder<Cabins_TMP> queryBuilder = cabins_tmpDao.queryBuilder().where(Cabins_TMPDao.Properties.Excursion.eq(excursionUniqId), Cabins_TMPDao.Properties.CruizeId.eq(cruizeID)).orderAsc(Cabins_TMPDao.Properties.CabinNumber);
            cabins_tmpList = queryBuilder.list();
            daoSession.clear();
...
//                            cabins_tmp.setExcursion(-1L);
    ...
    public ArrayList<Cabins_TMP> cabinByCruiseAndExcursionId(long cruise_uniqueId, long excursionId) {
        ...
        try {
            openReadableDb();
            Cabins_TMPDao cabins_tmpDao = daoSession.getCabins_TMPDao();
            QueryBuilder<Cabins_TMP> queryBuilder = cabins_tmpDao.queryBuilder().where(Cabins_TMPDao.Properties.CruizeId.eq(cruise_uniqueId), Cabins_TMPDao.Properties.Excursion.eq(excursionId));//.orderAsc(Excursions_TMPDao.Properties.Id);
    ...
    @Override
    public ArrayList<Cabins_TMP> cabinByCruiseAndExcursionId2(long cruise_uniqueId, long excursionId) {
        List<Cabins_TMP> cabin_tmpList = null;
        try {
            openReadableDb();
            Cabins_TMPDao cabins_tmpDao = daoSession.getCabins_TMPDao();
